module Unit.Semantics.HelloWorld (helloWorldSemAST) where
    ...
    ( Let
      [ FunDef "main" [] Nothing
        ( Expr
          ...
          ( SemTag
            { posn = AlexPn 11 1 12, typeInfo = NodeType ( SymType UnitType ) }
          )
        )
        ( SemTag
          { posn = AlexPn 4 1 5
